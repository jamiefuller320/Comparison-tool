...
`output/learned-qa-patterns.json` stores **confirmed junk phrases** from QA strip/thin actions (site chrome, policy TOC labels, parenting tips, admissions marketing). Those phrases feed ingest filters on later captures so the same junk is less likely to reappear. The active matching set is capped (**900**); a phrase enters that set only when it has been confirmed at least twice **or** is a long specific label (≥24 characters). A larger archive (**2000**) keeps demoted phrases with hit/recency stats, and selection uses **per-class floors** so chrome-heavy learning does not starve rarer junk classes. Legacy `byClass` orphans are merged back into the candidate pool on load. Further store-growth options (CLI cap knob, Aho-Corasick matcher, tighter `is_learnable_phrase`) live under **Qualitative learned-QA phrase store** in `DEFERRED_IDEAS.md`.
