...
| **Richer qualitative evidence layer** | Deeper researched qualitative context with user-accessible evidence (beyond current Ofsted/EYFSP footnotes); optional key phrases from Ofsted reports. | Partial (précis MVP + `tools/school-capture` Website evidence UI; Hampshire 12-school pilot with Cursor narratives; learned URL terms boost discovery only — not a self-improving LLM. Next: batch capture across Hampshire/packs, selective synth, resume/offset, term decay, scheduled qualitative loop) | North Star #2 |
