...
| **Richer qualitative evidence layer** | Deeper researched qualitative context with user-accessible evidence (beyond current Ofsted/EYFSP footnotes); optional key phrases from Ofsted reports. | Partial (Website evidence UI + Hampshire Cursor pilot; citation→URL learning; per-area `--only-missing` preserve; weekly loop `auto`/bounded Cursor). Still open: pack-wide roll-out, human feedback into gates, citation index alignment with UI footnotes | North Star #2 |
