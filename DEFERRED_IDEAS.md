...
| **Richer qualitative evidence layer** | Deeper researched qualitative context with user-accessible evidence (beyond current Ofsted/EYFSP footnotes); optional key phrases from Ofsted reports. | Partial (Website evidence UI + Hampshire pilot Cursor narratives + batch resume/`--skip-existing` upsert + selective synth gates + learned-term decay/stopwords + weekly `qualitative-loop` workflow). Still open: pack-wide roll-out, human feedback into gates, IDF/CMS-aware terms | North Star #2 |
