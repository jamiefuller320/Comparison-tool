...
| **Richer qualitative evidence layer** | Deeper researched qualitative context with user-accessible evidence (beyond current Ofsted/EYFSP footnotes); optional key phrases from Ofsted reports. | Partial (Website evidence UI + Hampshire capture expanding beyond pilot with Cursor narratives; batch resume with per-school upsert; selective synth; learned-term decay/stopwords + DF/IDF boosts; weekly `qualitative-loop`). Still open: pack-wide roll-out, human feedback into gates | North Star #2 |
