...
| **Richer qualitative evidence layer** | Deeper researched qualitative context with user-accessible evidence (beyond current Ofsted/EYFSP footnotes); optional key phrases from Ofsted reports. | Partial (coverage-first free crawl + deterministic narratives; Cursor reserved for rich-school polish; citation→URL learning). Still open: pack-wide roll-out, human feedback into gates, citation index alignment with UI footnotes | North Star #2 |
