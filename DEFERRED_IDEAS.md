...
| **Ofsted report précis engine** | Second-pass item: generate a short, parent-facing précis of each setting’s latest Ofsted report (verifiable quotes / footnote back to the report). Not needed before Hampshire age-climb. | Partial (`enrich-inspection-precis.py`: verbatim Ofsted/ISI PDF excerpts + footnote UI; widen coverage / refresh cadence still open) | User |
