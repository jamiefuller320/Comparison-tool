...
| **8. Client load: geo-lazy packs** | After seed-first progressive load + retries (hardening pass), only fetch packs for the postcode’s LA / neighbours instead of all ready SE packs (~20 MB). Keep static Pages hosting. Share-link URNs resolve via `packs/urn-lookup.json`. | **Shipped** | Seed-only boot in `CompareLoader`; `ensureAreaCoverageForDistrict` + neighbours; `ensureAreaCoverageForUrns` for `?schools=` |
| **8b. Detach qualitative from bulk index** | Full `qualitativeCapture` blobs → `/data/qualitative/{urn}.json` shards; schools-index keeps `qualitativeCaptureEnrichedAt` pointer only. Shortlist / content-review load on demand. | **Shipped** | `merge-qualitative.py` shard mode; `npm run publish:qualitative`; `qualitativeLoad.ts` |
| **9. Dynamic data API / CDN** | Move off “download every pack JSON in the browser” to an edge/API that returns a postcode-scoped slice (or versioned regional bundles on a real CDN). Only justify if geo-lazy static packs + qualitative shards are still too heavy or pack count grows nationally. | Deferred | After step 8 / 8b; do **not** jump here for flaky first loads alone |
...
| **2. London wave** | London borough packs `ready` in `manifest.json`; website qualitative pools now addressable | `auto` → `london`: parallel **Lambeth + Tower Hamlets** anchors, **60** new/stream, **12** stale refresh | **Open** (packs shipped; qualitative website roll-out continues via `loop:qualitative`) |
...
1. **Primary — maintain the shipped footprint:** stale re-screens (`refresh_stale_days`), daily **qualitative-quality-loop**, daily **qualitative-spotcheck-loop** (sample-capped source-vs-site fidelity after quality apply → safe chrome/PDF auto-learn → next quality apply; ethos stays human-gated), weekly **pack-quality-loop** (précis / ISI), heuristic QA + human junk flags (`npm run qa:human-flags`), optional **Cursor** on rich shortlist-likely schools (manual dispatch). Do not let ring expansion displace these budgets.
...
| **Second geography** | Widen beyond Hampshire via silent-merge packs. **South East + Dorset + London** is the **core** coverage region (`pack:southeast` / `pack:london`); Hampshire stays the sole maintained root. After core + London: **maintain + progressive adjacent rings** toward national search (`PROGRESSIVE_NATIONAL_PACK_BUILD_ORDER`) — not a second maintained seed. | Partial (SE+Dorset+London packs shipped; ring order documented; progressive rings not started) | User |
