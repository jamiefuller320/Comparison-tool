...
| *(none)* | `qualitative-spotcheck-loop` | Live HTML fetch; digests + safe chrome/PDF auto-learn into learned store; may `gh workflow run` quality loop (needs `actions: write`); no model secrets |
