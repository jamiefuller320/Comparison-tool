...
**On-demand area packs (backend collation):** outside Hampshire, parents can **Request area coverage** (exact DfE LA label) from **A school is missing**. That queues `repository_dispatch` `la-pack` → `scripts/build-la-pack.py`, which writes a schools index under `public/data/packs/{slug}/` without overwriting the Hampshire root. Ready packs in `public/data/packs/manifest.json` are **merged silently** into the map and search — there is no pack picker or `?pack=` mode. Packs remain the unit for deciding which LAs to harvest. EY depth for packs and an interest-weighted ingest→assess→improve loop are tracked in `DEFERRED_IDEAS.md`.
