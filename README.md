...
**Hampshire EY MVP (live in product):** Ofsted childcare MI for named Early Years Register full/sessional day care in Hampshire (`public/data/ey-providers-index.json`), Ofsted state-school grades joined onto GIAS nursery/infant/primary settings with an early-years intake (`npm run enrich:ey-schools`), plus England/Hampshire **EYFSP** area benchmarks (DfE does not publish provider-level EYFSP — use Ofsted for setting judgements and EYFSP only as area context). Refresh with `npm run harvest:ey`. Childminders deferred (names redacted in Ofsted MI).
...
- Matches comparison tables to selected stages: **Early years → Hampshire day care + school nursery/infant Ofsted + EYFSP area context**, **KS1 → LA / England phonics context** (DfE does not publish school-level phonics; KS1 TA is no longer collected), **KS2 → Year 6 tables**, **KS3/KS4 → GCSE / 16–18** (state and independent)
...
# or: npm run harvest:ey            # Hampshire Ofsted day care + EY school Ofsted + EYFSP
